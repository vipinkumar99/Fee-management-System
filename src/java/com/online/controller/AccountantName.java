 ...
 * @author vipin
 */
@WebServlet(name = "AccountantName", urlPatterns = {"/AccountantName"})
public class AccountantName extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
         ...
         request.getRequestDispatcher("AdminHeader.html").include(request, response);
            ...
            List<AccountantBeans> list=ViewAccountant.getRecordsByName(name);
            
           if(list.isEmpty())
            {
        request.setAttribute("AccountantError", "1");
