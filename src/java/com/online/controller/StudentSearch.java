 ...
 * @author vipin
 */
@WebServlet(name = "StudentSearch", urlPatterns = {"/StudentSearch"})
public class StudentSearch extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       request.getRequestDispatcher("AccountantHeader.html").include(request, response);
            ...
            List<StudentBeans> list=ViewStudentName.getRecordsByRegister(sregisterno);
            
           if(list.isEmpty())
            {
         
       request.setAttribute("StudentError", "8");
		...
		out.println("<h3 class='user'>Student detial of registration number:"+sregisterno+"</h3>");
        ...
        out.println("<a href='SearchStudent.html' class='btn btn-warning btn-lg' role='button' > Save </a> ");
