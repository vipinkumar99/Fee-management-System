 ...
 * @author vipin
 */
@WebServlet(name = "StudentEmail", urlPatterns = {"/StudentEmail"})
public class StudentEmail extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
         ...
         request.getRequestDispatcher("AccountantHeader.html").include(request, response);
            ...
            List<StudentBeans> list=ViewStudentName.getAllRecordsByEmail(email);
            
           if(list.isEmpty())
            {
        request.setAttribute("StudentError", "6");
